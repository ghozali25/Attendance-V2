...
<div x-data="{ show: @if($attributes->wire('model')->value()) $wire.entangle('{{ $attributes->wire('model')->value() }}'){{ $attributes->wire('model')->hasModifier('live') ? '.live' : '' }} @else false @endif }" x-on:close.stop="show = false; {{ $onclose }}"
